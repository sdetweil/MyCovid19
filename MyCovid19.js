...
Module.register("MyCovid19", {
      ...
      if (self.config.type == "countries") {
        for (var i in this.config.countries) {
          switch (this.config.countries[i].toLowerCase()) {
            ...
            default:
              self.config.countries[i] = self.config.countries[i].replace(
                " ",
                "_"
              );
            // code block
  ...
  getDom: function () {
    ...
    if (self.suspended == false) {
      ...
      for (var country_index in self.config[self.config.type]) {
        ...
        if (this.config.debug1)
          Log.log(
            self.config.chart_type +
              "=" +
              JSON.stringify(
                self.our_data[this_country][self.config.chart_type]
              )
          );
        ...
        var c = null;
        if (
          (canvas = document.getElementById("myChart_" + self.ourID)) != null
        ) {
          if (self.config.debug)
        ...
        if (
          ...
          canvas.style.height =
            self.config.height - self.config.attribution_label_height + "px";
  ...
  offlineTimer: (info) => {
    if (
      (canvas = document.getElementById("myChart_" + info.self.ourID)) != null
    ) {
      info.self.drawChart(info.self, info);
    } else
  ...
  drawChart: (self, info) => {
    ...
    attribution.innerText =
      "courtesy " + info.self.config.attribution_label[info.self.config.type];
      ...
      if (payload.id == self.ourID) {
        ...
        if (lastMoment.format("MM/DD/YYYY") !== currentMoment_date) {
          ...
          self.setTimerForNextRefresh(
            self,
            payload.config.newFileAvailableTimeofDay[payload.config.type],
            "hours"
          );
        }
        if (self.config.debug)
